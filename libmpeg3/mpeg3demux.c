...
/* Create space for bit */
			...
			if(demuxer->dump)
			{
				fprintf(stderr, " pcr_flag=%x time=%f\n", pcr_flag, demuxer->time);
			}
		}
		else
...
static int get_program_association_table(mpeg3_demuxer_t *demuxer)
{
	...
	if(demuxer->dump)
	{
		fprintf(stderr, " table_id=0x%x section_length=%d transport_stream_id=0x%x\n",
			demuxer->table_id,
...
/* Get Presentation Time stamps and Decoding Time Stamps */
	...
	if(demuxer->dump)
	{
		fprintf(stderr, " pts_dts_flags=0x%02x pts=%f dts=%f\n",
			pts_dts_flags,
			(double)*pts / 90000,
			(double)*dts / 90000);
	}
...
// AC3 audio
		...
		demuxer->got_audio = 1;
		demuxer->custom_audio_id = demuxer->pid;

		if(demuxer->read_all)
			demuxer->astream_table[demuxer->custom_audio_id] = AUDIO_AC3;
		if(demuxer->astream == -1)
		    demuxer->astream = demuxer->custom_audio_id;
		...
		if(demuxer->dump)
		{
			fprintf(stderr, "get_transport_pes_packet: offset=%jx 0x%x bytes AC3 custom_id=0x%x astream=0x%x do_audio=%d\n", 
				...
				demuxer->custom_audio_id,
				demuxer->astream,
				demuxer->do_audio);
		}

    	if((demuxer->custom_audio_id == demuxer->astream && 
			demuxer->do_audio) ||
			demuxer->read_all)
		{
	...
	if((demuxer->stream_id >> 4) == 0x0c || 
		(demuxer->stream_id >> 4) == 0x0d)
	{
// MPEG audio
		demuxer->custom_audio_id = demuxer->pid;
		demuxer->got_audio = 1;
...
/* Just pick the first available stream if no ID is set */
		if(demuxer->read_all)
			demuxer->astream_table[demuxer->custom_audio_id] = AUDIO_MPEG;
		if(demuxer->astream == -1)
		    demuxer->astream = demuxer->custom_audio_id;

		if(demuxer->dump)
		{
			fprintf(stderr, " 0x%x bytes MP2 audio\n", demuxer->raw_size - demuxer->raw_offset);
		}
	...
	if((demuxer->stream_id >> 4) == 0x0e)
	{
// Video
		demuxer->custom_video_id = demuxer->pid;
		demuxer->got_video = 1;
...
/* Just pick the first available stream if no ID is set */
		if(demuxer->read_all)
		{
			demuxer->vstream_table[demuxer->custom_video_id] = 1;
		}
		else
		if(demuxer->vstream == -1)
			demuxer->vstream = demuxer->custom_video_id;


		if(demuxer->dump)
		{
			fprintf(stderr, " 0x%x bytes video data\n", demuxer->raw_size - demuxer->raw_offset);
		}

		if((demuxer->custom_video_id == demuxer->vstream && 
			demuxer->do_video) ||
			demuxer->read_all)
		{
...
//printf("get_payload 1 pid=0x%x unit_start=%d\n", demuxer->pid, demuxer->payload_unit_start_indicator);
	...
	else
	{
		if(demuxer->dump)
		{
			fprintf(stderr, " 0x%x bytes elementary data\n", demuxer->raw_size - demuxer->raw_offset);
		}
 ...
 */
    	if(demuxer->pid == demuxer->audio_pid && 
			(demuxer->do_audio || demuxer->read_all))
		{
		...
		if(demuxer->pid == demuxer->video_pid && 
			(demuxer->do_video || demuxer->read_all))
		{
...
/* Packet size is known for transport streams */
	...
	demuxer->got_audio = 0;
	demuxer->got_video = 0;
	demuxer->custom_audio_id = -1;
	demuxer->custom_video_id = -1;
...
// if(!debug_fd) debug_fd = fopen("/tmp/test2.ac3", "w");
// fwrite(demuxer->audio_buffer + demuxer->audio_size,
// 1, 
// bytes, 
// debug_fd);
// 
// printf("get_program_payload %d wrote %d\n", __LINE__, bytes);
...
__LINE__, stream_id, bytes);
	...
	mpeg3io_read_data(demuxer->subtitle_buffer, 
		bytes, 
		title->fs);
	demuxer->subtitle_size = bytes;


	for(i = 0; i < demuxer->subtitle_size; )
	{
		if(!subtitle)
			subtitle = new_subtitle(demuxer, 
				stream_id, 
				demuxer->program_byte + i);
 ...
 */
			...
			memcpy(subtitle->data + subtitle->bytes_read, 
				...
				fragment);
			subtitle->bytes_read += fragment;
			i += fragment;


			if(subtitle->bytes_read >= subtitle->size)
			{
/* Test for valid subtitle */
				while(subtitle->bytes_read > 2 &&
					subtitle->data[subtitle->size - 1] != 0xff)
				{
/* Shift buffer and try new offsets */
//if(debug) fprintf(stderr, "handle_subtitle %d size=%d\n", __LINE__, subtitle->size);
					if(subtitle->size > 0) 
						memcpy(subtitle->data, subtitle->data + 1, subtitle->size - 1);
					subtitle->bytes_read--;
					subtitle->size = (subtitle->data[0] << 8) |
						subtitle->data[1];
					if(subtitle->size > subtitle->bytes_read)
					{
						subtitle->data = realloc(subtitle->data, subtitle->size);
						break;
					}
				}
			}
...
if(debug) fprintf(stderr, "handle_subtitle %d subtitle->data=%p\n", __LINE__, subtitle->data);
			subtitle->data = realloc(subtitle->data, 2);
if(debug) fprintf(stderr, "handle_subtitle %d i=%d subtitle=%p subtitle->data=%p subtitle->size=%d subtitle_size=%d bytes=%d\n", 
__LINE__, 
i,
subtitle,
subtitle->data,
subtitle->size,
demuxer->subtitle_size, 
bytes);

			while(subtitle->bytes_read < 2 &&
				i < bytes)
			{
				subtitle->data[subtitle->bytes_read++] = 
					demuxer->subtitle_buffer[i++];
			}

if(debug) fprintf(stderr, "handle_subtitle %d\n", __LINE__);
// Allocate true buffer
			if(subtitle->bytes_read >= 2)
			{
				subtitle->size = (subtitle->data[0] << 8) |
					subtitle->data[1];
				subtitle->data = realloc(subtitle->data, subtitle->size);
...
/* Skip unknown */
		...
		else
		{
			int pts_dts_flags;
/* Get MPEG-1 packet */
			while(!mpeg3io_eof(title->fs) && mpeg3io_next_char(title->fs) == 0xff)
			{
...
/* Now extract the payload. */
		if((demuxer->stream_id >> 4) == 0xc || 
			(demuxer->stream_id >> 4) == 0xd)
		{
//printf("get_program_pes_packet %d audio data\n", __LINE__);
...
/* Take first stream ID if -1 */
			...
			demuxer->got_audio = 1;
			demuxer->custom_audio_id = demuxer->stream_id & 0x0f;
				...
				demuxer->read_all)
			{
				...
				if(demuxer->dump)
				{
					fprintf(stderr, " MP2 audio data offset=%jx custom_audio_id=%x size=%x\n", 
						demuxer->program_byte, 
						demuxer->custom_audio_id,
						pes_packet_length);
				}
		...
		{
/* Video data */
//printf("get_program_pes_packet %d video data\n", __LINE__);
/* Take first stream ID if -1 */
			pes_packet_length -= mpeg3io_tell(title->fs) - pes_packet_start;

			demuxer->got_video = 1;
			demuxer->custom_video_id = demuxer->stream_id & 0x0f;
				...
				demuxer->read_all)
			{
				...
				if(demuxer->dump)
				{
					fprintf(stderr, " video offset=%jx custom_video_id=%x size=%x\n", 
						demuxer->program_byte,
						demuxer->custom_video_id,
						pes_packet_length);
				}
    		...
    		else 
			{
				if(demuxer->dump)
				{
					fprintf(stderr, " skipping video size=%x\n", pes_packet_length);
				}
    			mpeg3io_seek_relative(title->fs, pes_packet_length);
    		}
    	}
...
//__LINE__, stream_id, (int)pes_packet_length);
		...
		if((demuxer->stream_id == 0xbd || demuxer->stream_id == 0xbf) && 
			...
			((mpeg3io_next_char(title->fs) & 0xf0) == 0x80 ||
				(mpeg3io_next_char(title->fs) & 0xf0) == 0xa0))
		{
			int format;
/* DVD audio data */
//printf("get_program_pes_packet %d DVD audio data\n", __LINE__);
...
// Picks up bogus data if (& 0xf) or (& 0x7f)
			...
			demuxer->got_audio = 1;
			demuxer->custom_audio_id = demuxer->stream_id;


/* Take first stream ID if not building TOC. */
			if(demuxer->read_all)
				demuxer->astream_table[demuxer->custom_audio_id] = format;
			else
			if(demuxer->astream == -1)
				demuxer->astream = demuxer->custom_audio_id;
...
//printf("get_program_pes_packet 5 %x\n", decryption_offset);
				...
				if(demuxer->dump)
				{
					fprintf(stderr, " AC3 audio data size=%x\n", pes_packet_length);
				}
...
if(debug) fprintf(stderr, "mpeg3demux_read_program %d\n", __LINE__);
	...
	demuxer->program_byte = 
		mpeg3_absolute_to_program(demuxer, mpeg3io_tell(title->fs) +
			title->start_byte);
// fprintf(stderr, "mpeg3demux_read_program %d %ld\n", 
// __LINE__, (long)demuxer->program_byte);
	return result;
}
...
/* Allocate buffer + padding */
	...
	demuxer->subtitle_buffer = calloc(1, MPEG3_RAW_SIZE);
	demuxer->subtitle_allocated = MPEG3_RAW_SIZE;
	...
	int bytes)
{
	...
	if(demuxer->data_size + new_data_size >= demuxer->data_allocated)
	{
		demuxer->data_allocated = (demuxer->data_size + new_data_size) * 2;
