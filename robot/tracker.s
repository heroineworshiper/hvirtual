...
; initialize servos
	...
	bclr PACTL,x I4O5                       ; enable servo 5
...
detector_event:
		brclr PORTA,x IC1 + IC2 + IC3 detector_event_done

		ldaa PORTA,x                              ; detector is high
		anda IC1 + IC2 + IC3                      ; set the bits in the detector variable
		staa detector_high
...
; if the detector either transitioned or was high, this returns true
		...
		andb #IC1F + IC2F + IC3F    ; filter just the inputs
		orab detector_high          ; add continuous value
		clr detector_high           ; clear continuous value
		bclr TFLG1,x ~(IC1F + IC2F + IC3F)          ; clear transition flag
